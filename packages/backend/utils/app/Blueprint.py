...
class Blueprint(quart.Blueprint):
  ...
  def route(self, rule: str, **options: Any) -> Any:
    ...
    ``` code-block:: python
      bp: app.Blueprint = app.Blueprint("test-bp", __name__)
  ...
  def refactor_route(self, rule: str, **options: Any):
    ...
    This method has similar input args as :method:`Blueprint.route` and has very similar functionality. This method returns a :class:`app.Route` object.
    ...
    ```
    """
    route = app.RefactoredRoute(rule, **options)
    endpoint = options.pop("endpoint", None)
  ...
  def __get_blueprints__(cls) -> List[Union[importlib.machinery.ModuleSpec, str]]:
    ...
    :returns: List[Union[importlib.machinery.ModuleSpec, str]]: List of spec objects which can be inserted into :method:`app.Quart.load`
    """
    out_specs: List[Path] = []
