...
class Logger:
  ...
  def __file_log__(cls, context: Union[object, str, Any], severity: Optional[Union[str, int, SeverityChild]] = Severity.Fatal, max_bytes: int = int(2.56*1024**2)) -> None:
    ...
    *_, stack = inspect.stack()
    ...
    # Remove Color Text From Logs
    clean_context = re.sub(r"<\d{1,2};2;\d{1,3};\d{1,3};\d{1,3}}", "", str(context))
    
    # Solidify Output
    out = f"[{date_str}] [{time}] [{severity.name_short}] [{stack_parent_directory}/{stack_file_name}:{stack_file_line}] {clean_context}\n"
